...
Applications provide deep-copy factories for editable grid DTOs and atomic backend persistence for batch saves. Upload handlers implement the real destination and honor cancellation; the demonstration only reads local files. Scheduler dragging/resizing, all-day appointments and external calendar synchronization are not part of this change. The migration guide documents the .NET 10 minimum and matching Primitives package required by release builds.
